...
pub fn read_coordinates(path: &str) -> io::Result<Vec<(i64, i64, i64)>> 
{
    let file = fs::File::open(path)?;
    let reader = io::BufReader::new(file);
    ...
    for line_result in reader.lines() 
	{
        ...
        let third: i64 = parts[2].parse().expect("Failed to parse the third number");
        ranges.push((first, second, third));
    }
